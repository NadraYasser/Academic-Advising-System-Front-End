...
export function StatCard({ theme='blue', icon, value, label, sub, onClick, style }) {
  ...
  return (
    ...
    >
      ...
      {sub && <div style={{ fontSize:13, marginTop:3,fontWeight:500 }}>{sub}</div>}
    </div>
  );
}
...
export function CardTitle({ children }) {
  return <span style={{ fontSize:17, fontWeight:700, color:'var(--muted)' }}>{children}</span>;
}
...
/*  Attempt Badge  */
export function AttBadge({ n }) {
  const labels = ['1st','2nd','3rd','4th'];
  const colors = { 1:'var(--muted)', 2:'var(--amber)', 3:'var(--rose)' };
  const bdrColors = { 1:'var(--dim)', 2:'var(--amber)', 3:'var(--rose)' };
  return (
    <span style={{
      display:'inline-flex', padding:'2px 8px', borderRadius:5,
      fontSize:13, fontWeight:600,
      border:`1.5px solid ${bdrColors[n] || 'var(--rose)'}`,
      color: colors[n] || 'var(--rose)',
    }}>
      {labels[n-1] || `${n}th`}
    </span>
  );
...
export function GPABar({ sem, gpa }) {
  ...
  const color = gpa >= 3.7 ? 'var(--green)' : gpa >= 3 ? 'var(--blue2)' : gpa >= 2.33 ? 'var(--amber)':  gpa >= 2 ? '#f97316' : 'var(--rose)';
  return (
