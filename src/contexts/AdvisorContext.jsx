...
// import { ADVISORS, STUDENTS, HISTORY, GPA_HIST, NOTES, SLOTS, APPTS } from '../services/advisorMockData';
...
// // ── Mock (يستخدم فقط لو USE_MOCK = true) ───────────────────
// const MOCK_ADVISOR = ADVISORS[0];
...
//     case 'SET_SELECTED_STUDENT': return { ...state, selectedStudent: action.payload };
//     case 'SET_STUDENT_HISTORY':  return { ...state, studentHistory: action.payload };
//     case 'SET_STUDENT_GPA':      return { ...state, studentGPAHist: action.payload };
//     case 'SET_STUDENT_NOTES':    return { ...state, studentNotes: action.payload };
//     case 'SET_LOADING':          return { ...state, loading: action.payload };
//     case 'SET_STUDENT_LOADING':  return { ...state, studentLoading: action.payload };

//     case 'ADD_NOTE':
//       return { ...state, studentNotes: [action.payload, ...state.studentNotes] };
...
//       return {
//         ...state,
//         appts: state.appts.map(a => a.id === action.id ? { ...a, status: 'noshow' } : a)
//       };
...
//       dispatch({ type: 'SET_STUDENTS', payload: STUDENTS.filter(s => s.advisorId === MOCK_ADVISOR.id) });
//       dispatch({ type: 'SET_SLOTS',    payload: SLOTS.filter(s => s.advisorId === MOCK_ADVISOR.id) });
//       dispatch({ type: 'SET_APPTS',    payload: APPTS.filter(a => a.aid === MOCK_ADVISOR.id) });
...
//       const [students, appts] = await Promise.all([
//         apiGet('/advisor/students'),
//         apiGet('/advisor/appointments')
//       ]);

//       dispatch({
//         type: 'SET_STUDENTS',
//         payload: Array.isArray(students) ? students : (students?.data ?? [])
//       });

//       dispatch({
//         type: 'SET_APPTS',
//         payload: Array.isArray(appts) ? appts : (appts?.data ?? [])
//       });

//     } catch (err) {
//       console.error('Boot error:', err);
//       toast('❌ Failed to load data — check backend', 'err');
//     }
...
//     if (USE_MOCK) {
//       dispatch({ type: 'SET_STUDENT_HISTORY', payload: HISTORY[student.id] || [] });
//       dispatch({ type: 'SET_STUDENT_GPA',     payload: GPA_HIST[student.id] || [] });
//       dispatch({ type: 'SET_STUDENT_NOTES',   payload: NOTES[student.id] || [] });
//     } else {
//       try {
//         const [hist, notes] = await Promise.all([
//           apiGet(`/advisor/students/${student.id}/history`),
//           apiGet(`/advisor/students/${student.id}/notes`)
//         ]);

//         dispatch({ type: 'SET_STUDENT_HISTORY', payload: Array.isArray(hist) ? hist : [] });
//         dispatch({ type: 'SET_STUDENT_NOTES',   payload: Array.isArray(notes) ? notes : [] });
